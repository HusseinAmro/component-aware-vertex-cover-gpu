...
static void usage()
{
    fprintf(stderr,
            ...
            "                                  - Default : 0\n"
            "\n"
            "   -q <Global WorkList Size>          Used to set the global work list size. Must\n"
            "                                  be and Integer and a power of 2. If not set by \n"
            "                                  the user it will be determined by the code \n"
            "                                  according to the abilities of the gpu and the\n"
            "                                  size of the graph.\n"
            "                                  - Default if no other constraints : 2^19 (512k)\n"
            "\n"
            "   -t <WorkList Threashold>           Used to set the global work list threashold.\n"
            ...
            "                                  argument won't have any effect. If not set by \n"
            "                                  the user it will be determined by the code \n"
            "                                  according to the available number of blocks.\n"
            "\n"
            ...
            "\n"
            "   -c <CompArray Size>               Used to set the size of the compArray for \n"
            "                                  the Hybrid version. Must be an Integer between \n"
            "                                  3 and 1,000,000,000 (1_Billion).\n"
            "                                  Enter 0 for not using component method.\n"
            "                                  -Default: 700,000,000 (700_Million).\n"
            "\n"
            "\n");
}
...
static Config parseArgs(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "v:f:o:i:k:q:t:d:b:g:n:c:h")) >= 0)
    {
        ...
        case 'n':
            ...
            break;
        case 'c':
            config.compArraySize = atoi(optarg);
            break;
        case 'h':
            usage();
            exit(0);
