...
static unsigned int compSequentialParameterized(CSRGraph graph, unsigned int minimum, int *rootVertexDegrees, unsigned int* toReachK, unsigned int* kFound, unsigned int k, unsigned int left, unsigned int right)
{
    Stack stack;
    ...
    while (stack.top != -1)
    {
        ...
        else
        {
            if (maxDegree == 0)
            {
                *toReachK -= minimum-numDeletedVertices;
            ...
            else
            {
                ...
                if (inside_counter==1) {
                    ...
                    unsigned int minim;
                    unsigned int initialMin = minimum-numDeletedVertices;
                    *toReachK += inside_counter*initialMin-initialMin;
                    for(unsigned int i=1; i <= inside_counter; ++i) {
                        ...
                        if (numDeletedVertices<minimum) {
                            ...
                            *toReachK -= initialMin-minim;
                            if(first_vertex == 2){
                                numDeletedVertices += (inside_compSize + 1) / 2;
                            }else if(first_vertex == inside_compSize - 1){
                                numDeletedVertices += first_vertex;
                            }else{
                                numDeletedVertices += compSequentialParameterized(graph, minim, inside_vertexDegrees, toReachK, kFound, k, inside_l, inside_r);
                            } 
                            
                        if (*kFound) {break;}
                        } else {
                            *toReachK -= (inside_counter-i)*initialMin+numDeletedVertices-minimum;
                            break;
                        }                             
                    }
...
void SequentialParameterized(CSRGraph graph, unsigned int k, unsigned int *kFound)
{
    ...
    unsigned int toReachK = counter*(k+1);
    unsigned int minim;
    int *vertexDegrees = (int*)calloc(sizeof(int),graph.vertexNum);
    ...
    for(unsigned int i=1; i <= counter; ++i) {
        ...
        if (rootNumDeletedVertices<k+1) {
            minim = (k+1-rootNumDeletedVertices<compSize-1)?k+1-rootNumDeletedVertices:compSize-1;
            toReachK -= k+1-minim;
            // For Debugging Purposes
            //printf("toReachK = %d\n",toReachK);
            if (toReachK<=k || *kFound) {
                break;
            }
            if(first_vertex == 2){
                ...
                rootNumDeletedVertices += compSequentialParameterized(graph, minim, vertexDegrees, &toReachK, kFound, k, left, right);
            }
            
            // For Debugging Purposes
            //printf("minim = %d\n",minim);
            //printf("rootNumDeletedVertices = %d\n",rootNumDeletedVertices);
        } else {
            break;
        }
    }
